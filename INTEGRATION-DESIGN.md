...
> ## ⚠️ 이 문서는 2026-08-24 시점 설계안이다 — 그대로 구현되지 않았다
>
> 2026-09-04에 한 번 재기획하면서(`CLAUDE.md`의 "v1 범위 결정") 이 문서의 여러
> 결정이 뒤집혔다. **지금 코드가 실제로 무엇을 하는지는 이 문서가 아니라
> `README.md`/`CLAUDE.md`/`HANDOFF.md`를 볼 것.** 이 문서는 "그때 왜 이렇게
> 하기로 했는가"를 보여주는 역사적 기록으로만 남긴다.
>
> 아래는 이 설계서 이후 뒤집힌 주요 결정이다.
>
> | 이 문서의 결정 | 실제로는 | 근거 |
> |---|---|---|
> | §4 파이프라인의 `core/verify` 검산, `postable` 게이트 | **제거.** 모든 생성 결과는 무조건 검토대기로 들어가고 사람이 승인 여부를 정한다 | `CLAUDE.md` v1 범위 결정 #1 |
> | §4의 HTML 리포트(`data/reports/`) | **제거.** 관련 모듈 없음 | 위와 동일 |
> | §4·§7의 3영업일 리마인더(`follow_up_due`) → 커리어데이 자동 이관 | **제거.** 커리어데이는 CLI `careerday-prepare` 명령으로 사람이 수동 트리거 | 위와 동일 |
> | §6.7 구글 시트 성과 기록, Notion 미러 | **제거.** 관련 어댑터(`adapters/sheets`, `adapters/notion`) 없음 | 위와 동일 |
> | §5.3의 호출 직렬화·일일 호출 상한 가드 | **제거.** `adapters/llm/guards.mjs`에는 인증 검사만 있다 | 위와 동일, `guards.mjs` 확인 |
> | §5.1의 `maxTurns: 1` | **4로 변경.** 실제 크기 제안서 PDF는 구조화 출력 재시도가 필요해 1로는 막혔다(2026-09 재현) | `CLAUDE.md` "모델 호출" 절 |
> | §6.4 승인 지점(Slack 버튼 하나) | **그대로 유지됨** — 이 결정은 안 뒤집혔다 | — |
> | §6.6 커리어데이 Playwright 자동 입력(`careerday-runner.mjs`) | **그대로 유지됨** — `login`/`fill`/`inspect` 명령 실재 | `apps/careerday-runner.mjs` |
>
> "고객사 명시 요구사항"처럼 이 문서 작성 이후 새로 생긴 결정은 여기 없다 —
> `HANDOFF.md`에서 날짜순으로 따라가는 게 낫다.
